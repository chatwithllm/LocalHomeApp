"""Run a controlled LocalHomeApp storage persistence demo."""
...
def main() -> None:
    ...
    try:
        ...
        print(f'purchase_history_count={len(rows)}')
        for row in rows[:10]:
            print(f"merchant={row['merchant_name']} date={row['receipt_date']} item={row['item_name']} total={row['line_total']}")
    finally:
        connection.close()
