...
def test_purchase_history_repository_lists_line_items(tmp_path: Path) -> None:
    ...
    try:
        ...
        connection.execute(
            "INSERT INTO receipt_parses (parse_id, intake_id, ocr_run_id, merchant_name, receipt_date, parse_status) VALUES (?, ?, ?, ?, ?, ?)",
            ("parse-001", "intake-001", "ocr-001", "Kroger", "03/15/26", "completed"),
        )
        connection.execute(
        ...
        connection.commit()
        repository = PurchaseHistoryRepository(connection)
        rows = repository.list_line_items()
        assert len(rows) == 1
